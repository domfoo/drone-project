...
* **Local Computer:** Runs the AI detection model and transmits commands via UDP using Netcat (nc)
* **Raspberry Pi:** Receives network packets, maintains a MAVLink heartbeat with the FC, bridges signals from Local Computer to Flight Controller and controls the servo via GPIO
...
* Bridge Script: Save `command_listener.py` to `/home/tpu/`
...
2. Ensure the camera feed is visible (verify `CAM_INDEX` in real_mission.py)
...
When `run_inference` returns True, the system automatically executes:
...
* **Script Updates:** If you update `command_listener.py`, run `sudo systemctl restart drone_bridge.service` on the Pi
...
* Run `real_mission.py` with AI on demo laptop
