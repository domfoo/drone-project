...
def trigger_drone_action(command):
    """
    Sends a UDP command to the Raspberry Pi bridge.
    Automatically selects the network interface.
    """
    # Create a UDP socket
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    
    # Set a 1-second timeout so a network hiccup won't freeze your AI inference
    sock.settimeout(1.0)
    
    try:
        sock.sendto(command.encode(), (RASPBERRY_PI_IP, UDP_PORT))
...
def trigger_drone_action_mac(command):
